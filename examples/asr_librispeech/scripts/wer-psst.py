...
# Function to read and process the file, filtering lines containing 'BNT'
def read_and_process_file(file_path, filter_keyword="BNT"):
    with open(file_path, 'r') as file:
        lines = file.readlines()

    total_lines = len(lines)  # Count all lines initially
    processed_lines = []
    for line in lines:
        parts = line.split('\t')
        if len(parts) > 1 and filter_keyword in parts[0]:  # Only include lines with 'BNT'
            processed_lines.append(parts[1].strip())
    
    filtered_lines_count = len(processed_lines)  # Count lines after filtering
    print(f"Total lines in file: {total_lines}")
    print(f"Lines containing '{filter_keyword}': {filtered_lines_count}")
