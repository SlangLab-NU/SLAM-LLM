...
class SpeechDatasetJsonl(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # j: Add phoneme extraction
        phoneme = data_dict.get("phoneme", None)
        ...
        prompt = "Give me the following information about the audio [Transcript, Phoneme Transcript]"
        # if prompt is None:
        ...
        # Modified answer template
        output_prompt = "{"
        output_prompt += f'  "Transcript": "{target}", '    # Hardcode Transcript
        output_prompt += f'  "Phoneme": "{phoneme}", '      # Hardcode Phoneme
        output_prompt = output_prompt.rstrip(', ') + "}"    # Remove trailing comma and close

        answer = output_prompt
