...
def main(kwargs: DictConfig):
    ...
    del kwargs["peft_config"]  # j: update peft
    ...
    # log_config.log_file = f"/work/van-speech-nlp/jindaznb/jslpnb/mllm_experiments/slam-llm/out/log/{current_time}.txt"
    ...
    if not (train_config.enable_fsdp or train_config.enable_ddp) or rank == 0:
        if log_config.use_wandb:
            ...
            wandb_config = {"train_config": train_config, "fsdp_config": fsdp_config,
                            "model_config": model_config, "log_config": log_config}
            wandb.init(dir=log_config.wandb_dir, entity=log_config.wandb_entity_name,
                       project=log_config.wandb_project_name, name=log_config.wandb_exp_name, config=wandb_config)
    ...
    if train_config.enable_fsdp:
        ...
        model = FSDP(
            model,
            # (FIX:MZY): Using my_auto_wrapping_policy whether peft or not. This will avoid model shard type check error of requires_grad mismatching.
    ...
    # j: load checkpoint for optimizer and scheduler again
    ...
    if ckpt_path is not None:
        ...
        if 'lr_scheduler_state' in checkpoint:
            ...
            logger.info(f"Current scheduler step (last_epoch): {current_step}")
        else:
            logger.warning("Learning rate scheduler state not found in checkpoint.")
